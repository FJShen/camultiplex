...
namespace camera{
	...
	for(int i=0; i<N; i++){
	    ...
	    //depth_pub[i] = rh.advertise<camultiplex::BitLayeredImage>(s_d+std::to_string(i), 8);
	}
	...
	sensor_msgs::Image depth_high_msg;
	sensor_msgs::Image depth_low_msg;
	...
	std::vector<uint8_t> depth_high_image(1*pixel_amount);
	std::vector<uint8_t> depth_low_image(1*pixel_amount);
	...
	for(long int idx = 0; idx<2 * pixel_amount * sizeof(uint8_t); idx+=2){
	    depth_low_image[idx/2] = pixel_ptr[idx];
	    depth_high_image[idx/2] = pixel_ptr[idx + 1];
	}
...
/*        depth_msg.header.frame_id = std::to_string(seq);//this is the sequence number since start of the programme
	...
	depth_high_msg.header.frame_id = std::to_string(seq);//this is the sequence number since start of the programme
	depth_high_msg.header.stamp.sec = second ;
	depth_high_msg.header.stamp.nsec = nanosecond;
	depth_high_msg.data = depth_high_image;
	depth_high_msg.height = height;
	depth_high_msg.width = width;
	depth_high_msg.encoding = sensor_msgs::image_encodings::MONO8;
	depth_high_msg.step = width*1; //each pixel is 1 bytes, so step, which stands for row length in bytes, should be one times "width".

	
	depth_low_msg.header.frame_id = std::to_string(seq);//this is the sequence number since start of the programme
	depth_low_msg.header.stamp.sec = second ;
	depth_low_msg.header.stamp.nsec = nanosecond;
	depth_low_msg.data = depth_low_image;
	depth_low_msg.height = height;
	depth_low_msg.width = width;
	depth_low_msg.encoding = sensor_msgs::image_encodings::MONO8;
	depth_low_msg.step = width*1; //each pixel is 1 bytes, so step, which stands for row length in bytes, should be one times "width".
	...
	//this part can be optimized with c++11 move semantics
	//camultiplex::BitLayeredImage compound;
	//compound.HighEightBitLayer = depth_high_msg;
	//compound.LowEightBitLayer = depth_low_msg;
	//depth_pub[channel].publish(compound);
	depth_pub[channel].publish(depth_low_msg);
